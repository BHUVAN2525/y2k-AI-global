...
class DocumentAnalyzer:
    ...
    def analyze_document(self, file_path):
        ...
        try:
            if magic:
                ...
                # Fallback to simple extension check or generic type
                file_type = "Generic document"
                mime_type = "application/octet-stream"
            logger.info(f"Analyzing file: {file_path}, Type: {file_type}, MIME: {mime_type}")
        except Exception as e:
            logger.error(f"Error determining file type with magic: {e}")
            file_type = "Unknown"
            mime_type = "application/octet-stream"
    ...
    def _analyze_office_document(self, file_path, results):
        ...
        if not olefile:
            results["suspicious_indicators"].append("Missing olefile library for deep Office analysis")
            return
